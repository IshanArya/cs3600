...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        actionValues = list(
            map(lambda action: self.values[(state, action)], legalActions))
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        "*** YOUR CODE HERE ***"
        subQ = reward + self.discount * self.computeValueFromQValues(nextState)
        oldQ = self.getQValue(state, action)

        q = (1 - self.alpha) * oldQ + self.alpha * subQ
