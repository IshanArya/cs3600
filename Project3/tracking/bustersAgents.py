...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for beliefDist in livingGhostPositionDistributions:
            maxGhost, maxPosition = max(beliefDist.items(), key=lambda (x, y): y)

            dist = self.distancer.getDistance(pacmanPosition, maxGhost)

            if closestGhostDistance == None or dist < closestGhostDistance:
                closestGhostDistance = dist
                closestGhostPosition = maxGhost

        minAction = min(legal, key=lambda action: self.distancer.getDistance(
            Actions.getSuccessor(pacmanPosition, action), closestGhostPosition))
