...
class ExactInference(InferenceModule):
        ...
        if noisyDistance is None:
            ...
            for legalPosition in self.legalPositions:
                trueDistance = util.manhattanDistance(
                    legalPosition, pacmanPosition)
                allPossible[legalPosition] = emissionModel[trueDistance] * \
                    self.beliefs[legalPosition]
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        "*** YOUR CODE HERE ***"
        newEstimates = util.Counter()
        for oldPos in self.legalPositions:
            newPosDist = self.getPositionDistribution(
                self.setGhostPosition(gameState, oldPos))
            for newPos, prob in newPosDist.items():
                totalProbability = self.beliefs[oldPos] * prob
                newEstimates[newPos] += totalProbability

        self.beliefs = newEstimates
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            #     self.getJailPosition] * self.numParticles
        else:
            newEstimates = util.Counter()
            ...
            for particle in self.particles:
                ...
                newEstimates[particle] = newEstimates[particle] + \
                    emissionModel[dist]
            newEstimates.normalize()

            if newEstimates.totalCount() == 0:
                self.initializeUniformly(gameState)
            else:
                self.particles = [util.sample(newEstimates)
                                  for i in range(self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            currentDistribution = self.getPositionDistribution(
                self.setGhostPosition(gameState, oldPos))
            newParticles.append(util.sample(currentDistribution))
