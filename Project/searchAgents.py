    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.EAST, Directions.WEST, Directions.NORTH, Directions.SOUTH]:
            ...
            nextState = (nextPosition,
                         visitedCorners.difference([nextPosition]))
...
def cornersHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    leftover_corners = set(state[1])
    if len(state[1]) <= 0:
        return 0
    closest_point = min(
        state[1], key=lambda x: util.manhattanDistance(x, state[0]))
    leftover_corners.remove(closest_point)
    sumOfCorners = [closest_point]
    while len(leftover_corners) > 0:
        nextCorner = min(
            leftover_corners, key=lambda x: util.manhattanDistance(x, sumOfCorners[-1]))
        leftover_corners.remove(nextCorner)
        sumOfCorners.append(nextCorner)
    sumOfStuff = util.manhattanDistance(state[0], closest_point)
    for i in range(1, len(sumOfCorners) + 1):
        if(i >= len(sumOfCorners)):
            break
        sumOfStuff = sumOfStuff + \
            util.manhattanDistance(sumOfCorners[i], sumOfCorners[i - 1])
    return sumOfStuff
...
def foodHeuristic(state, problem):
    ...
    b = foodGrid.asList()[0]
    maxDistance = 0
    for aPoint in foodGrid.asList():
        for bPoint in foodGrid.asList():
            currentDistance = util.manhattanDistance(aPoint, bPoint)
            if currentDistance > maxDistance:
                a = aPoint
                b = bPoint
                maxDistance = currentDistance

    a = util.manhattanDistance(position, a)
    b = util.manhattanDistance(position, b)
    return maxDistance + min(a, b)
